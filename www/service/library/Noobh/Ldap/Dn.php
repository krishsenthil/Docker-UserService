...
/**
 * Noobh_Ldap_Dn provides an API for DN manipulation
 *
 * @category   Noobh
 * @package    Noobh_Ldap
 * @copyright  Copyright (c) Collash Inc
 * @license    Collash Inc
 */
class Noobh_Ldap_Dn implements ArrayAccess
{
    ...
    public static function factory($dn, $caseFold = null)
    {
        if (is_array($dn)) {
            ...
            /**
             * Noobh_Ldap_Exception
             */
            require_once 'Noobh/Ldap/Exception.php';
            throw new Noobh_Ldap_Exception(null, 'Invalid argument type for $dn');
        }
    }
     ...
     * @return array
     * @throws Noobh_Ldap_Exception if DN has no RDN (empty array)
     */
    ...
    public function getParentDn($levelUp = 1)
    {
        ...
        if ($levelUp < 1 || $levelUp >= count($this->_dn)) {
             ...
             */
            require_once 'Noobh/Ldap/Exception.php';
            throw new Noobh_Ldap_Exception(null, 'Cannot retrieve parent DN with given $levelUp');
        }
     ...
     * @param  string $caseFold
     * @return array
     * @throws Noobh_Ldap_Exception if index is illegal
     */
     ...
     * @param  int   $index
     * @param  array $value
     * @return Noobh_Ldap_Dn Provides a fluent interface
     * @throws Noobh_Ldap_Exception if index is illegal
     */
    ...
    protected function _assertIndex($index)
    {
        if (!is_int($index)) {
             ...
             */
            require_once 'Noobh/Ldap/Exception.php';
            throw new Noobh_Ldap_Exception(null, 'Parameter $index must be an integer');
        }
        if ($index < 0 || $index >= count($this->_dn)) {
             ...
             */
            require_once 'Noobh/Ldap/Exception.php';
            throw new Noobh_Ldap_Exception(null, 'Parameter $index out of bounds');
        }
        return true;
    }
    ...
    protected static function _assertRdn(array $value)
    {
        if (count($value)<1) {
             ...
             */
            require_once 'Noobh/Ldap/Exception.php';
            throw new Noobh_Ldap_Exception(null, 'RDN Array is malformed: it must have at least one item');
        }
        ...
        foreach (array_keys($value) as $key) {
            if (!is_string($key)) {
                 ...
                 */
                require_once 'Noobh/Ldap/Exception.php';
                throw new Noobh_Ldap_Exception(null, 'RDN Array is malformed: it must use string keys');
            }
        }
    }
    ...
    public static function unescapeValue($values = array())
    {
        ...
        foreach ($values as $key => $val) {
            ...
            $val = str_replace(array('\\\\', '\,', '\+', '\"', '\<', '\>', '\;', '\#', '\='),
                array('\\', ',', '+', '"', '<', '>', ';', '#', '=', ), $val);
            $values[$key] = Noobh_Ldap_Converter::hex32ToAsc($val);
        }
        return (count($values) == 1) ? $values[0] : $values;
    }
        ...
        $caseFold = self::ATTR_CASEFOLD_NONE)
    {
        ...
        if (!self::checkDn($dn, $k, $v, $caseFold)) {
             ...
             */
            require_once 'Noobh/Ldap/Exception.php';
            throw new Noobh_Ldap_Exception(null, 'DN is malformed');
        }
        $ret = array();
        for ($i = 0; $i < count($k); $i++) {
     ...
     *
     * @param  string|Noobh_Ldap_Dn $childDn
    ...
    public static function isChildOf($childDn, $parentDn)
    {
        try {
            $keys = array();
            $vals = array();
            if ($childDn instanceof Noobh_Ldap_Dn) {
                $cdn = $childDn->toArray(Noobh_Ldap_Dn::ATTR_CASEFOLD_LOWER);
            } else {
                $cdn = self::explodeDn($childDn, $keys, $vals, Noobh_Ldap_Dn::ATTR_CASEFOLD_LOWER);
            }
            if ($parentDn instanceof Noobh_Ldap_Dn) {
                $pdn = $parentDn->toArray(Noobh_Ldap_Dn::ATTR_CASEFOLD_LOWER);
            } else {
                $pdn = self::explodeDn($parentDn, $keys, $vals, Noobh_Ldap_Dn::ATTR_CASEFOLD_LOWER);
            }
        }
