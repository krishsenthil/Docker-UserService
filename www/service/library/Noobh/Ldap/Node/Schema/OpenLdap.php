...
/**
 * @see Noobh_Ldap_Node_Schema_AttributeType_OpenLdap
 */
require_once 'Noobh/Ldap/Node/Schema/AttributeType/OpenLdap.php';
/**
 * @see Noobh_Ldap_Node_Schema_ObjectClass_OpenLdap
 */
require_once 'Noobh/Ldap/Node/Schema/ObjectClass/OpenLdap.php';

/**
 * Noobh_Ldap_Node_Schema_OpenLdap provides a simple data-container for the Schema node of
 * an OpenLDAP server.
 *
 * @category   Noobh
 * @package    Noobh_Ldap
 ...
 * @license    Collash Inc
 */
class Noobh_Ldap_Node_Schema_OpenLdap extends Noobh_Ldap_Node_Schema
{
    /**
     * The attribute Types
     *
    ...
    protected function _loadAttributeTypes()
    {
        ...
        foreach ($this->getAttribute('attributeTypes') as $value) {
            ...
            $val = new Noobh_Ldap_Node_Schema_AttributeType_OpenLdap($val);
    ...
    protected function _loadObjectClasses()
    {
        ...
        foreach ($this->getAttribute('objectClasses') as $value) {
            ...
            $val = new Noobh_Ldap_Node_Schema_ObjectClass_OpenLdap($val);
     ...
     *
     * @param Noobh_Ldap_Node_Schema_Item $node
     * @param array                      $repository
     */
    protected function _resolveInheritance(Noobh_Ldap_Node_Schema_Item $node, array $repository)
    {
        $data = $node->getData();
        $parents = $data['sup'];
            ...
            if (in_array($token, $noValue)) {
                ...
                if ($data[$token] == '(') {
                    ...
                    while ($tmp = array_shift($tokens)) {
                        if ($tmp == ')') break;
                        if ($tmp != '$') {
                            $data[$token][] = Noobh_Ldap_Attribute::convertFromLdapValue($tmp);
                        }
                    }
                } else {
                    $data[$token] = Noobh_Ldap_Attribute::convertFromLdapValue($data[$token]);
                }
