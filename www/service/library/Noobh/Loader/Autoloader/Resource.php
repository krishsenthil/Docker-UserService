 ...
 * @license    Collash Inc
 */
/** Noobh_Loader_Autoloader_Interface */
require_once 'Noobh/Loader/Autoloader/Interface.php';
/**
 * Static methods for loading classes and files.
 *
 * @uses       Noobh_Loader_Autoloader
 * @package    Noobh_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) Collash Inc
 * @license    Collash Inc
 */
class Noobh_Loader_Autoloader_Resource implements Noobh_Loader_Autoloader_Interface
{
    /**
     ...
     * Constructor
     *
     * @param  array|Noobh_Config $options Configuration options for resource autoloader
     * @return void
     */
    public function __construct($options)
    {
        //@todo: Currenlty not supporting config files but need to support
        if ($options instanceof Noobh_Config) {
            $options = $options->toArray();
        }        
        if (!is_array($options)) {
            require_once 'Noobh/Loader/Exception.php';
            throw new Noobh_Loader_Exception('Options must be passed to resource loader constructor');
        }
        ...
        if ((null === $namespace)
            ...
            require_once 'Noobh/Loader/Exception.php';
            throw new Noobh_Loader_Exception('Resource loader requires both a namespace and a base path for initialization');
        }

        if (!empty($namespace)) {
            $namespace .= '_';
        }
        require_once 'Noobh/Loader/Autoloader.php';
        Noobh_Loader_Autoloader::getInstance()->unshiftAutoloader($this, $namespace);
    }
     ...
     * syntax. Example:
     * <code>
     * $loader = new Noobh_Loader_Autoloader_Resource(array(
     *     'namespace' => 'Stuff_',
     ...
     * @param  array $args
     * @return mixed
     * @throws Noobh_Loader_Exception if method not beginning with 'get' or not matching a valid resource type is called
     */
    public function __call($method, $args)
    {
        if ('get' == substr($method, 0, 3)) {
            ...
            if (!$this->hasResourceType($type)) {
                require_once 'Noobh/Loader/Exception.php';
                throw new Noobh_Loader_Exception("Invalid resource type $type; cannot load resource");
            }
            if (empty($args)) {
                require_once 'Noobh/Loader/Exception.php';
                throw new Noobh_Loader_Exception("Cannot load resources; no resource specified");
            }
        ...
        require_once 'Noobh/Loader/Exception.php';
        throw new Noobh_Loader_Exception("Method '$method' is not supported");
    }
    ...
    public function getClassPath($class)
    {
        ...
        if (Noobh_Loader::isReadable($classPath)) {
            return $classPath;
        }
     ...
     *
     * @param  array $options
     * @return Noobh_Loader_Autoloader_Resource
     */
    ...
    public function addResourceType($type, $path, $namespace = null)
    {
        ...
        if (!isset($this->_resourceTypes[$type])) {
            if (null === $namespace) {
                require_once 'Noobh/Loader/Exception.php';
                throw new Noobh_Loader_Exception('Initial definition of a resource type must include a namespace');
            }
            $namespaceTopLevel = $this->getNamespace();
        ...
        if (!is_string($path)) {
            require_once 'Noobh/Loader/Exception.php';
            throw new Noobh_Loader_Exception('Invalid path specification provided; must be string');
        }
    ...
    public function addResourceTypes(array $types)
    {
        foreach ($types as $type => $spec) {
            if (!is_array($spec)) {
                require_once 'Noobh/Loader/Exception.php';
                throw new Noobh_Loader_Exception('addResourceTypes() expects an array of arrays');
            }
            if (!isset($spec['path'])) {
                require_once 'Noobh/Loader/Exception.php';
                throw new Noobh_Loader_Exception('addResourceTypes() expects each array to include a paths element');
            }
            $paths  = $spec['path'];
            $namespace = null;
     ...
     *
     * @see    Noobh_Loader_Autoloader_Resource::addResourceTypes()
     * @param  array $types
     * @return Noobh_Loader_Autoloader_Resource
     */
     ...
     * @param  string $type
     * @return object
     * @throws Noobh_Loader_Exception if resource type not specified or invalid
     */
    public function load($resource, $type = null)
    {
        if (null === $type) {
            ...
            if (empty($type)) {
                require_once 'Noobh/Loader/Exception.php';
                throw new Noobh_Loader_Exception('No resource type specified');
            }
        }
        if (!$this->hasResourceType($type)) {
            require_once 'Noobh/Loader/Exception.php';
            throw new Noobh_Loader_Exception('Invalid resource type specified');
        }
        $namespace = $this->_resourceTypes[$type]['namespace'];
