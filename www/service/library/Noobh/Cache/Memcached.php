 ...
 * namespace and impose default expiration time.
 * 
 * 
 ...
 * @date Aug 28, 2012
 */
class Noobh_Cache_Memcached extends Memcached
{ 
    /**
     ...
     * then use this key for writting into same memcahe
     * location
     * @var {string}
     */
    const FRAMEWORK_DEFAULT_APP_ID = 'Noobh_FRAMEWORK_DEFAULT_CACHE_CONSTANT';
    
    /**
     * Default AppID is used which is overriden by
     * all other application with their appId for app
     * specific memory allocation
     * @var {string}
     */
    const DEFAULT_APP_ID = 'Noobh_DEFAULT';
    
		/**
     * Default persistent connection ID
     * @var {string}
     */
		const DEFAULT_MEMCACHED_PERSISTENT_ID = 'Noobh_DEFAULT_MEMCACHED_PERSISTENT_ID';
    
		/**
     * Group all cached data  by Noobh framework
     * with following namespace
     * @var {string}
     */
    private $_namespaceFrameWork = 'Noobh_FRAMEWORK';
    ...
    /**
     * If fallover is enabled then after connection
     ...
     * @param {string} $appId, UniqueId for Noobh Applications
     * @return {Noobh_Cache_Memcached}
     */
    public function __construct($config = NULL,$appId = NULL){				
        ...
        /**
         * If $config is not present then default to the
     ...
     * 
     * If memcache is disbaled then this method will return false
     * 
     * @access private
     * @param void
     * @throws Exception
    ...
    protected function _getConnection(){
				...
				foreach ($servers as $server) {
						...
						//Cehck server is already added to connection.
						$addServer = TRUE;
